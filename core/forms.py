...
class RegisterForm(forms.ModelForm):
    ...
    def clean_invitation_code_input(self):
        code = self.cleaned_data.get('invitation_code_input')
        # On vérifie si le code existe, sauf si c'est le tout premier utilisateur (admin)
        if not User.objects.filter(invitation_code=code).exists():
             # Exception pour le premier utilisateur ou debug
             if User.objects.count() == 0: 
                 return code
             raise forms.ValidationError("Ce code d'invitation est invalide.")
        return code
...
class TaskForm(forms.ModelForm):
    class Meta:
        model = Task
        fields = ['title', 'description', 'platform', 'category', 'link', 'is_automatic']
        widgets = {
